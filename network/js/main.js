...
function configSigmaElements(config) {
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> ' + config.legend['colorLabel'+x++] + ' (' + sigInst.clusters[b].length + ' levélíró)</a></div>');
    //a.sort();
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>>Adjon meg legalább három betűt a kereséshez.</i>");
        else {
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        sigInst.draw(2, 2, 2, 2);
        $GP.info_name.html(false);
        $GP.info_data.hide();
        $GP.info_p.html("Levélírók:");
        $GP.info_link.find("ul").html(f.join(""));
        $GP.info.animate({width:'show'},350);
        $GP.search.clean();
		$GP.cluster.hide();
